...
export class FexoraClient {
  ...
  private async request<T>(
    ...
    if (!res.ok) {
      ...
      return { data: null as T, success: false, error: error.error ?? res.statusText };
    }
  ...
  async upload<T>(path: string, formData: FormData, options?: RequestOptions) {
    ...
    if (!res.ok) {
      const error = await res.json().catch(() => ({ error: res.statusText }));
      return { data: null as T, success: false, error: error.error ?? res.statusText } as ApiResponse<T>;
    }
...
export { FexoraClient as default };
